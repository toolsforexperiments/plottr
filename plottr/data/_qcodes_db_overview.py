"""
Local (vendored) copy of the QCoDeS ``get_db_overview`` implementation.

This module is a private fallback used by :mod:`plottr.data.qcodes_db_overview`
for QCoDeS versions that do not yet expose ``qcodes.dataset.get_db_overview``.
Its content is kept as an exact copy of the upstream QCoDeS implementation,
except that the (private) QCoDeS SQLite imports are done lazily inside
:func:`get_db_overview` rather than at module import time.
...
thousands of runs almost instantly.
"""
...
def get_db_overview(
    ...
    * If neither is available the count is reported as ``0`` (unknown).
    ...
    """
    # These are (technically private) QCoDeS SQLite helpers; import them lazily
    # so that importing this module does not depend on QCoDeS internals.
    import sqlite3
