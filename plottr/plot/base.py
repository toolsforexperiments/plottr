    ...
    def dataIsComplex(self, dependentName: Optional[str] = None) -> bool:
        ...
        if dependentName is None:
            for d in self.data.dependents():
                if np.issubdtype(self.data.data_vals(d), np.complexfloating):
                    return True
        else:
            if np.issubdtype(self.data.data_vals(dependentName), np.complexfloating):
                return True
...
class AutoFigureMaker:
    ...
    def _splitComplexData(self, plotItem: PlotItem) -> List[PlotItem]:
        ...
        if not np.issubdtype(plotItem.data[-1], np.complexfloating):
            return [plotItem]
